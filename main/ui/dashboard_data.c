                            ...
                            const fishduino_feeder_t *feeder, const fishduino_settings_t *settings)
{
    ...
    if (!settings->heater.enabled) {
        snprintf(out->heater_relay_text, sizeof(out->heater_relay_text), "HEATER RELAY OFF");
    } else if (hs.heating) {
        snprintf(out->heater_relay_text, sizeof(out->heater_relay_text), "HEATER RELAY ON / HEATING");
    } else {
        snprintf(out->heater_relay_text, sizeof(out->heater_relay_text), "HEATER RELAY OFF / IDLE");
    }
    snprintf(out->heater_shelly_text, sizeof(out->heater_shelly_text), "SHELLY HEATER %s",
    ...
    if (settings->shelly_filter.enabled) {
        ...
        snprintf(out->filter_health_text, sizeof(out->filter_health_text), "DISABLED");
    }
    ...
    if (settings->shelly_co2.enabled) {
        ...
        out->co2_on = ss.co2_desired_on && ss.co2_block_reason == CO2_BLOCK_NONE;
    ...
    if (ss.co2_block_reason != CO2_BLOCK_NONE && !out->co2_on) {
        snprintf(out->co2_block_text, sizeof(out->co2_block_text), "%s",
                 fishduino_co2_safety_reason_text(ss.co2_block_reason));
    } else {
    ...
    if (out->feeder_configured && out->feeder_scheduled && now.valid_time) {
    ...
    out->led_shelly_filter = settings->shelly_filter.enabled
                                 ? (ss.filter_alarm == FISHDUINO_FILTER_ALARM_OFFLINE ? DASHBOARD_LED_FAULT
                                                                                      : DASHBOARD_LED_OK)
                                 : DASHBOARD_LED_NA;
    out->led_shelly_heater = out->heater_online ? DASHBOARD_LED_OK : DASHBOARD_LED_FAULT;
    ...
    if (settings->co2.enabled) {
        format_minutes(settings->co2.on_min, out->light_sunrise_text, sizeof(out->light_sunrise_text));
        format_minutes(settings->co2.off_min, out->light_sunset_text, sizeof(out->light_sunset_text));
    } else {
        snprintf(out->light_sunrise_text, sizeof(out->light_sunrise_text), "--:--");
        snprintf(out->light_sunset_text, sizeof(out->light_sunset_text), "--:--");
    }

    out->led_alerts = (ss.filter_alarm != FISHDUINO_FILTER_ALARM_NONE || hs.alarm != HEATER_ALARM_NONE ||
                       out->water_alert >= WATER_ALERT_WARNING || out->reminders_any)
                          ? DASHBOARD_LED_WARN
                          : DASHBOARD_LED_OK;
    ...
    } else if (ss.filter_alarm == FISHDUINO_FILTER_ALARM_OFF && ss.alert_blink_on) {
        out->banner_visible = true;
