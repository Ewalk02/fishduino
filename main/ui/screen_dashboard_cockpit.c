...
static void fishduino_cockpit_dashboard_build_compact(lv_obj_t *parent, fishduino_cockpit_handles_t *out)
{
    ...
    out->banner = make_panel(parent, 8, 4, 464, 28);
    ...
    lv_obj_align(out->lbl_brand, LV_ALIGN_TOP_MID, 0, 34);

    cockpit_gauge_create(&out->gauge_filter, parent, 6, 52, 118, "FILTER", "WATTS",
    ...
    cockpit_gauge_create(&out->gauge_temp, parent, 128, 44, 224, "TEMP", "TANK F",
    ...
    cockpit_gauge_create(&out->gauge_co2, parent, 356, 52, 118, "CO2", "INJECTION",
    ...
    cockpit_gauge_create(&out->gauge_feeder, parent, 6, 302, 104, "FEEDER", "SCHEDULE",
                         GAUGE_START, GAUGE_END, 0.0f, 1.0f);
    ...
    out->panel_trend = make_panel(parent, 116, 302, 356, 104);
    ...
    lv_obj_set_size(out->chart_temp, 340, 68);
    ...
    out->chart_temp_series = lv_chart_add_series(out->chart_temp, cockpit_color_cyan(), LV_CHART_AXIS_PRIMARY_Y);

    out->panel_water = make_panel(parent, 6, 414, 150, 92);
    ...
    make_water_column(out->panel_water, 4, "pH", &out->lbl_water_ph, &out->badge_water_ph);
    make_water_column(out->panel_water, 38, "NH3", &out->lbl_water_nh3, &out->badge_water_nh3);
    make_water_column(out->panel_water, 72, "NO2", &out->lbl_water_no2, &out->badge_water_no2);
    make_water_column(out->panel_water, 106, "NO3", &out->lbl_water_no3, &out->badge_water_no3);
    ...
    lv_obj_set_size(out->btn_water_view, 72, 22);
    lv_obj_align(out->btn_water_view, LV_ALIGN_BOTTOM_RIGHT, 0, 0);

    out->panel_reminders = make_panel(parent, 162, 414, 150, 92);
    ...
    lv_obj_align(out->lbl_reminders_next, LV_ALIGN_BOTTOM_LEFT, 0, 18);
    out->btn_reminders_view = make_text_btn(out->panel_reminders, "VIEW ALL", tap_reminders_cb);
    lv_obj_set_size(out->btn_reminders_view, 72, 22);
    lv_obj_align(out->btn_reminders_view, LV_ALIGN_BOTTOM_RIGHT, 0, 0);

    out->panel_clock = make_panel(parent, 318, 414, 154, 92);
    ...
    lv_obj_align(out->lbl_clock_date, LV_ALIGN_BOTTOM_LEFT, 0, 18);
    out->badge_mode = lv_obj_create(out->panel_clock);
    lv_obj_remove_style_all(out->badge_mode);
    lv_obj_set_size(out->badge_mode, 64, 22);
    ...
    out->panel_systems = make_panel(parent, 6, 514, 232, 108);
    ...
    struct {
        ...
        {"BLE Central", 116, 20, &out->led_ble},
        {"Feeder", 116, 38, &out->led_feeder},
        {"Lighting", 116, 56, &out->led_light},
        {"Alerts", 116, 74, &out->led_alerts},
    };
    ...
    out->panel_light = make_panel(parent, 244, 514, 228, 108);
    ...
    lv_obj_set_size(out->bar_light, 200, 10);
    ...
    lv_obj_set_size(out->nav_bar, 468, 50);
    lv_obj_set_pos(out->nav_bar, 6, 632);
    lv_obj_set_flex_flow(out->nav_bar, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(out->nav_bar, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    ...
    for (int i = 0; i < NAV_COUNT; i++) {
        ...
        lv_obj_set_size(out->nav_btns[i], 72, 40);
