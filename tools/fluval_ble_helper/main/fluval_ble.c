...
typedef struct {
    ble_job_type_t type;
    uint8_t data[FLUVAL_SET_CHANNELS_LEN];
    size_t len;
    fluval_ack_type_t expect_ack;
} ble_job_t;
...
typedef struct {
    ...
    fluval_state_t state;
    SemaphoreHandle_t state_mutex;
    EventGroupHandle_t wait_events;

    fluval_ack_type_t pending_ack;
    bool waiting_ack;
    bool waiting_status;
...
{
    return name != NULL && strcmp(name, FLUVAL_TARGET_NAME) == 0;
}
...
static void update_state_from_parsed(const fluval_state_t *parsed)
{
...
static void handle_notify(struct os_mbuf *om)
{
    ...
    uint8_t buf[FLUVAL_NOTIFY_MAX_LEN] = {0};
    ...
    if (esp_log_level_get(TAG) >= ESP_LOG_DEBUG) {
        char hex[FLUVAL_NOTIFY_MAX_LEN * 3 + 1] = {0};
        size_t off = 0;
    ...
    fluval_state_t parsed = {0};
    fluval_ack_type_t ack = fluval_parse_ack_packet(buf, (size_t)len, &parsed);

    switch (ack) {
    case FLUVAL_ACK_MODE_MANUAL:
    case FLUVAL_ACK_MODE_AUTO:
    case FLUVAL_ACK_SET_CHANNELS:
        if (s_ble.waiting_ack &&
            (s_ble.pending_ack == FLUVAL_ACK_NONE || s_ble.pending_ack == ack)) {
            xEventGroupSetBits(s_ble.wait_events, FLUVAL_EVT_ACK);
        }
        break;
    case FLUVAL_ACK_STATUS:
        update_state_from_parsed(&parsed);
        ESP_LOGI(TAG, "status: mode=%s avg=%u P=%u B=%u CW=%u W=%u WW=%u", fluval_mode_to_string(parsed.mode),
                                 ...
                                 void *arg)
{
    ...
    uint8_t query[FLUVAL_STATUS_QUERY_LEN];
    size_t qlen = fluval_build_status_query(query, sizeof(query));
    if (qlen > 0) {
...
static int scan_start(void)
{
    ...
    s_ble.have_adv_target = false;
    ESP_LOGI(TAG, "scanning for %s", FLUVAL_TARGET_NAME);
...
static void update_stale(void)
{
    ...
    if (s_ble.state.last_update_ms == 0) {
        s_ble.state.valid = false;
    } else {
        uint32_t age = now_ms() - s_ble.state.last_update_ms;
        if (age > FLUVAL_STALE_TIMEOUT_MS) {
            s_ble.state.valid = false;
        }
    }
    state_unlock();
...
static void maybe_poll_status(void)
{
    ...
    uint32_t t = now_ms();
    if (s_ble.last_poll_ms != 0 && (t - s_ble.last_poll_ms) < FLUVAL_POLL_INTERVAL_MS) {
        return;
    }
...
static esp_err_t send_and_wait_ack_status(const uint8_t *data, size_t len, fluval_ack_type_t expect_ack)
{
    if (!fluval_ble_is_connected()) {
    ...
    esp_err_t err = wait_bits(FLUVAL_EVT_ACK, FLUVAL_CMD_WAIT_MS);
    s_ble.waiting_ack = false;
    if (err != ESP_OK) {
        return err;
    }
    ...
    err = wait_bits(FLUVAL_EVT_STATUS, FLUVAL_CMD_WAIT_MS);
    s_ble.waiting_status = false;
    return err;
}
...
esp_err_t fluval_ble_wait_status(fluval_state_t *out, uint32_t timeout_ms)
{
    ...
    esp_err_t err = fluval_ble_request_status();
    if (err != ESP_OK) {
        s_ble.waiting_status = false;
        return err;
    }
    ...
    if (out != NULL) {
        return fluval_ble_get_state(out);
    }
    return ESP_OK;
}
...
{
    uint8_t cmd[FLUVAL_SET_MANUAL_LEN];
    size_t len = fluval_build_set_manual(cmd, sizeof(cmd));
    if (len == 0) {
        return ESP_FAIL;
    }
    return send_and_wait_ack_status(cmd, len, FLUVAL_ACK_MODE_MANUAL);
}

esp_err_t fluval_ble_set_mode_auto(void)
{
    uint8_t cmd[FLUVAL_SET_AUTO_LEN];
    size_t len = fluval_build_set_auto(cmd, sizeof(cmd));
    if (len == 0) {
        return ESP_FAIL;
    }
    return send_and_wait_ack_status(cmd, len, FLUVAL_ACK_MODE_AUTO);
}

esp_err_t fluval_ble_set_channels(uint8_t pink, uint8_t blue, uint8_t cold_white, uint8_t white, uint8_t warm_white)
{
    uint8_t cmd[FLUVAL_SET_CHANNELS_LEN];
    size_t len = fluval_build_set_channels(pink, blue, cold_white, white, warm_white, cmd, sizeof(cmd));
    if (len == 0) {
        return ESP_FAIL;
    }
    return send_and_wait_ack_status(cmd, len, FLUVAL_ACK_SET_CHANNELS);
}

esp_err_t fluval_ble_set_all(uint8_t percent)
{
    uint8_t p = fluval_clamp_percent(percent);
    return fluval_ble_set_channels(p, p, p, p, p);
}
